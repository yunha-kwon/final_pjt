<template>
    <div class="container">
      ...
      <div class="product-list">
        <h1 v-if="store.saveoptions.length > 0" class="product-title">금융 상품 코드 : {{ store.saveoptions[0].fin_prdt_nm }}</h1>
        <hr class="divider">
        <ProductSaveOption v-for="saveoption in filteredOptions" :key="saveoption.id" :saveoption="saveoption" />
      </div>
    </div>
  </template>
  ...
  import ProductSaveOption from '@/components/SaveProduct/ProductSaveOption.vue'
  ...
  const availableSaveTrmOptions = computed(() => {
    const uniqueSaveTrms = new Set(store.saveoptions.map(option => option.save_trm))
  ...
  const filteredOptions = computed(() => {
    return store.saveoptions.filter(option => option.save_trm == selectedSaveTrm.value)
  })
  </script>
